...
import { MoreVertical, Bot, Plus, Trash2, Book, Terminal } from 'lucide-react'
...
export default function DashboardPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen font-sans" style={{ backgroundColor: '#0f0f0f', color: '#ededed' }}>
      
      {/* ── 1. Global Navbar ── */}
      <Navbar userEmail={userEmail} activePage="dashboard" onLogout={handleLogout} />

      {/* ── 2. Content Container (Sidebar + Main) ── */}
          ...
          <div className="p-8 max-w-5xl mx-auto w-full flex-1">
            ...
            <div className="flex items-center justify-between mb-10">
              ...
              <Link href="/dashboard/new">
                <button disabled={bots.length >= 3} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 20px', borderRadius: 6, fontWeight: 600, fontSize: 13, backgroundColor: '#ededed', color: '#0f0f0f', border: 'none', cursor: bots.length >= 3 ? 'not-allowed' : 'pointer', opacity: bots.length >= 3 ? 0.5 : 1 }} className="hover:opacity-90 transition-opacity">
